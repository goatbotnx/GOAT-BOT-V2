...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
	...
	for (const folderModules of folders) {
		const makeColor = folderModules == "cmds" ?
			createLine("NX LOAD COMMANDS") :
			createLine("NX LOAD COMMANDS EVENT");
		...
		for (const file of Files) {
			...
			try {
				...
				// ——————————————— CHECK ENV GLOBAL ——————————————— //
				if (envGlobal) {
					if (typeof envGlobal != "NX object" || typeof envGlobal == "object" && Array.isArray(envGlobal))
			...
			loading.info('NX LOADED', `${colors.green(`${commandLoadSuccess}`)}${commandError.length ? `, ${colors.red(`${commandError.length}`)}` : ''}`);
		}
		console.log("\r");
		if (commandError.length > 0) {
			log.err("NX LOADED", getText('loadScripts', 'loadScriptsError', colors.yellow(text)));
